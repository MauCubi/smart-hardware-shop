...
export const ComponentListCard = ({ product }: Props) => {
  return (
    <div className='w-full max-w-sm bg-gray-900 border border-[#0A84FF] hover:-translate-y-0.5 duration-300 hover:shadow-2xl hover:shadow-gray-600 ease-in-out rounded-lg shadow-sm px-3 pb-5'>      
        <Image
          className='object-fit rounded-t-lg self-center place-self-center w-auto xl:w-full h-[100] xl:h-[250]'
          src={product.image}
          alt='product image'
          width={500}
          height={500}
      ...
      <div className='flex flex-col px-5 mt-4 items-center h-[30]'>
        <a href='#'>
          <h5 className='text-sm xl:text-lg font-semibold tracking-tight text-[#F1F1F1]'>
            {product.name}
          </h5>
        </a>
      </div>

      <div className='flex flex-col justify-end items-center h-fit px-6 md:px-3 xl:px-6 xl:mt-15 gap-3'>
        <span className='text-sm xl:text-3xl font-bold text-[#F1F1F1]'>
